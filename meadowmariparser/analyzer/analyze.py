...
def analyze(freqListFile=None, paradigmFile='paradigms.txt', lexFile='lexemes.txt', lexRulesFile='lex_rules.txt',
    ...
    # m.parse('къэкӏощт', printOut=True)
    # m.parse('щыӏэныгъэ', printOut=True)
    # m.parse('фай', printOut=True)
    # m.parse('мыжъохэп', printOut=True)
    # m.parse('марие', printOut=True)
    # m.parse('сыриш', printOut=True)
    # m.parse('непэрэ', printOut=True)
    # m.parse('тыгъосэрэ', printOut=True)
    # m.parse('тыгъуасэ', printOut=True)
    # m.parse('шырэ', printOut=True)
    # m.parse('текӏоныгъэр', printOut=True)
    # m.parse('мин', printOut=True)
    # m.parse('шачфтомс', printOut=True)
    # return
